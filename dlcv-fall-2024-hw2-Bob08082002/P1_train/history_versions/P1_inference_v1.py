...
from P1_train.modules_v1 import UNet_conditional  # use empty __init__.py s.t. UNet_conditional can be imported
...
def Sampling_specify_digit(net, alpha, alpha_hat, dataset_label, digit, w = 0,  T = 1000, N = 50, image_size = 32):
    ...
    for t_idx in range(T-1, -1, -1): # for t_idx = T-1,...,0  # since python is 0-indexing
        ...
        with torch.no_grad():
           ...
           epsilon_x_t_t_cond = net(x_t, t, y, dataset_label)  # since model and inputs are on device, epsilon_x_t_t_cond also on device
           # Predict the noise epsilon(x_t, t) with "unconditional" generate
           y = None
           dataset_label = None
           epsilon_x_t_t_uncond = net(x_t, t, y, dataset_label)
    ...
    return preprocess_and_resize(x_t) # return n generated image (N*C*H*W) # resize back to 28*28
...
def infer_and_save(net, alpha, alpha_hat, num_classes, output_image_folder):
    ...
    for dataset_label in [0, 1]:  # 0: MNISTM, 1: SVHN
        for image_label in range(num_classes):
            ...
            gen_images_uint8 = Sampling_specify_digit(net, alpha, alpha_hat, dataset_label=dataset_label, digit=image_label) # gen_images_uint8 is on cpu
            gen_images_uint8.to(torch.device("cpu")) # 以防萬一
...
def save_tensor_images(tensor, output_folder, prefix='image'):
    # Ensure the output folder exists
    os.makedirs(output_folder, exist_ok=True)

    # Loop over the batch dimension (N)
    for i in range(tensor.size(0)):
        # Convert tensor to numpy array and transpose to (H, W, C)
        img = tensor[i].permute(1, 2, 0).cpu().numpy()
        
        # Create filename with index
        filename = os.path.join(output_folder, f"{prefix}_{i}.png")
...
if __name__ == '__main__':
    ...
    net = UNet_conditional(size=32, time_dim=256, num_classes=10, device=device) # !!!!!!!!!! cpu, gpu !!!!!!!!!!
    net.load_state_dict(torch.load('checkpoint_model/P1/P1_model_ver1_3.pth', map_location="cpu"), strict=False) # !! cpu, gpu !!
    net = net.to(device)
    net.eval()
