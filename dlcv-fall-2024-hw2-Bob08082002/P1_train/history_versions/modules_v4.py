...
# version 4: combined 2 datasets to 1 dataset + change model + T = 400
...
class Down(nn.Module):
    """MODIFIED VERSION 4: combine two dataset, which has 20 classes"""
...
class UNet_conditional(nn.Module):
    ...
    def __init__(self, c_in=3, c_out=3, size=28, time_dim=256, num_classes=20, device="cuda"):
        ...
        self.inc = DoubleConv(c_in, 128) # size
        self.down1 = Down(128, 256) # reduce size by 2, first arg is in_channel, second arg is out_channel
        self.sa1 = SelfAttention(256, int(size/2)) # size/2 
        self.down2 = Down(256, 512)
        self.sa2 = SelfAttention(512, int(size/4)) # size/4 
        ...
        #self.up1 = Up(512, 128)    # with corresponding shortcut connected(sa2)
        #self.sa4 = SelfAttention(128, int(size/4))
        self.up2 = Up(512, 128)    # with corresponding shortcut connected(sa1)
        self.sa5 = SelfAttention(128, int(size/2))
        self.up3 = Up(256, 128)    # with corresponding shortcut connected(inc)
        self.sa6 = SelfAttention(128, int(size))
