...
class Down(nn.Module):
    """MODIFIED VERSION 5: combine two dataset, which has 20 classes"""
    def __init__(self, in_channels, out_channels, IsDownSample=True, emb_dim=256):
        super().__init__()
        if IsDownSample:
...
class Up(nn.Module):
    """MODIFIED VERSION 5: combine two dataset, which has 20 classes"""
    def __init__(self, in_channels, out_channels, IsUpSample=True, emb_dim=256):
        super().__init__()
        if IsUpSample:
...
class UNet_conditional(nn.Module):
    ...
    def __init__(self, c_in=3, c_out=3, size=28, time_dim=256, num_classes=20, device="cuda"):
        ...
        self.down3 = Down(256, 256, IsDownSample=False)
        self.sa3 = SelfAttention(256, int(size/4)) # size/4 
        ...
        # added layers in ver 5
        self.up0 = Up(512, 256, IsUpSample=False)   
        self.sa0 = SelfAttention(256, int(size/4))

        self.up1 = Up(512, 128, IsUpSample=False)   # upsample size by 2, first arg is in_channel, second arg is out_channel
    ...
    def forward(self, x, t=None, y=None):
        ...
        # added layers in ver 5
        x = self.up0(x5, x4, t) # with corresponding shortcut connected
        x = self.sa0(x)

        x = self.up1(x, x3, t) # with corresponding shortcut connected
        x = self.sa4(x)
        x = self.up2(x, x2, t) # with corresponding shortcut connected
        x = self.sa5(x)
