...
if __name__ == "__main__": 
    ...
    # ============================================ INFERENCE LOOP ============================================
    for source, concept_prompts in enumerate(prompts_list): # source = 0 or 1
        for prompt_index, prompts in enumerate(concept_prompts): # prompt_index = 0, 1, ... (private may have more prompts)
            data = [BATCH_SIZE * [prompts]] # one prompt, ex: Manhattan skyline in the style of <new2>.

            # if folder is not existed, then create it.
            if not os.path.exists(os.path.join(output_image_folder, str(source), str(prompt_index))):
                os.makedirs(os.path.join(output_image_folder, str(source), str(prompt_index)), exist_ok=True)  # Create the folder
                    ...
                    for n in trange(opt.n_iter, desc="Sampling"):
                        for prompts in tqdm(data, desc="data"):
                            ...
                            for x_sample in x_checked_image_torch:
                                ...
                                img.save(os.path.join(output_image_folder, str(source), str(prompt_index), f"source{source}_prompt{prompt_index}_{n}.png"))