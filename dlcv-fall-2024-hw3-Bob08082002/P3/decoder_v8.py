...
class Attention(nn.Module):
    ...
    def forward(self, x, return_weights=False):
        ...
        #print(att.shape) # torch.Size([1, 12, 258, 258])
        output = self.c_proj((att @ v).transpose(1, 2).contiguous().view(B, T, C))
        if return_weights: # if return_weights, retrun attention weight 
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        # get attention weights and save it in attn_weights
        attn_output, attn_weights = self.attn(self.ln_1(x), return_weights=True) # set return_weights=True to get attention weight
        self.attn_weights = attn_weights  

        x = x + attn_output            # x shape = (BS, K+257, 768)
