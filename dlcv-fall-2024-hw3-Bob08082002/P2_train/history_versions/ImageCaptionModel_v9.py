...
import torch
import timm
from decoder_v9 import Config, Decoder
...
class ImageCaptionModel(nn.Module):
    ...
    def __init__(self, vision_encoder, text_decoder):
        ...
        self.projection_layer = nn.Linear(1280, 768) #self.decoder_cfg.n_embd = 768, project to same dim for concat 


    def forward(self, token_id, images):
        """ image.shape = BS*3*336*336, token_id.shape = BS*K, note that token_id is padded output from tokenizer"""
        # img_feature.shape = torch.Size([BS, 577, 1280]), where 257 = Patch^2 + 1, 1 is for class embedding
        img_feature = self.encoder.forward_features(images)
        # img_embed.shape = torch.Size([BS, 577, 768])
        img_embed = self.projection_layer(img_feature)

        # token_id.shape = (BS, K)
        # img_embed.shape = (BS, 577, 768)
...
if __name__=="__main__":
    ...
    input_imgs = torch.rand(7, 3, 336, 336)
