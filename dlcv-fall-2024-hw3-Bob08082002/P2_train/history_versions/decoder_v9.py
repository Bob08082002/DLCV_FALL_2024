...
class Block(nn.Module):
    ...
    def forward(self, x):
        # x shape = (BS, K+577, 768)
        # pass concated text_embed and img_embed to self-attetion and MLP
        x = x + self.attn(self.ln_1(x)) # x shape = (BS, K+577, 768)
        x = x + self.mlp(self.ln_2(x))  # x shape = (BS, K+577, 768) 
        return x
...
class Decoder(nn.Module):
    ...
    def forward(self, token_id: Tensor, img_embed: Tensor):
        ...
        # text_embed shape = (BS, K, 768), img_embed shape = (BS, 577, 768)
        # concat text_embed and img_embed along dim=1
        #x = torch.cat((text_embed, img_embed), dim=1)  # x shape = (BS, K+577, 768) # version 1
        x = torch.cat((img_embed, text_embed), dim=1)  # x shape = (BS, K+577, 768) # version 2

        
        x = self.lm_head(self.transformer.ln_f(self.transformer.h(x))) # x.shape = (BS, K+577, 50257)
